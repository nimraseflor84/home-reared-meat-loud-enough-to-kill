...
# Riff Slicer: Kleiner, Latzhose, weisses Shirt, wilder Bart/Haare (6. ganz rechts)
# Passive: Durchdringt mehrere Gegner
# Ultimate: Shred Solo (8 String-Blades mit Bounces)

# Riff Slide Dash Trail
const TRAIL_DURATION: float = 1.5
const TRAIL_TICK: float = 0.15
const TRAIL_RADIUS: float = 28.0
# Trail entries: {pos: Vector2, age: float, hit: Array}
var _slide_trail: Array = []
var _trail_tick_timer: float = 0.0
...
func _ready() -> void:
	...
	base_damage = 22.0
	attack_speed = 0.85
	# Shred Solo: 5s Cooldown laut Spec
	ultimate_cooldown = 5.0
	pierce = 1
...
func _process(delta: float) -> void:
	...
	# Trail-Eintraege altern lassen
	if not _slide_trail.is_empty():
		var i: int = _slide_trail.size() - 1
		while i >= 0:
			_slide_trail[i]["age"] += delta
			if _slide_trail[i]["age"] >= TRAIL_DURATION:
				_slide_trail.remove_at(i)
			i -= 1
		queue_redraw()

func _physics_process(delta: float) -> void:
	super._physics_process(delta)
	# Trail-Damage-Tick: alle aktiven Trail-Punkte machen Schaden an durchlaufenden Feinden
	if _slide_trail.is_empty():
		return
	_trail_tick_timer -= delta
	if _trail_tick_timer > 0.0:
		return
	_trail_tick_timer = TRAIL_TICK
	var dmg: float = get_total_damage() * 0.4
	var enemies = get_tree().get_nodes_in_group("enemies")
	for trail in _slide_trail:
		var tpos: Vector2 = trail["pos"]
		var hit_list: Array = trail["hit"]
		for e in enemies:
			if not is_instance_valid(e) or e in hit_list:
				continue
			if tpos.distance_to(e.global_position) < TRAIL_RADIUS:
				hit_list.append(e)
				if e.has_method("take_damage"):
					e.take_damage(dmg, self)

func _on_dash_start() -> void:
	# Trail-Punkt am Startpunkt setzen
	_slide_trail.append({"pos": global_position, "age": 0.0, "hit": []})

func _on_dash_tick(delta: float) -> void:
	# Trail kontinuierlich entlang des Dash-Pfads aufbauen
	if _slide_trail.is_empty():
		_slide_trail.append({"pos": global_position, "age": 0.0, "hit": []})
		return
	var last_pos: Vector2 = _slide_trail[-1]["pos"]
	if global_position.distance_to(last_pos) > 18.0:
		_slide_trail.append({"pos": global_position, "age": 0.0, "hit": []})
...
func _draw() -> void:
	if _death_anim >= 0.0:
		_draw_death()
		return
	# Riff Slide Trail: String-Klingen am Boden zeichnen
	if not _slide_trail.is_empty():
		for trail in _slide_trail:
			var t_age: float = trail["age"]
			var fade: float = clamp(1.0 - t_age / TRAIL_DURATION, 0.0, 1.0)
			if fade <= 0.0:
				continue
			var offs: Vector2 = (trail["pos"] as Vector2) - global_position
			# Kreuzweise zwei Klingen mit metallic-Look
			var blade_col = Color(0.9, 0.85, 0.4, fade * 0.85)
			var blade_inner = Color(1.0, 0.95, 0.7, fade * 0.7)
			# Erste Klinge
			draw_line(offs + Vector2(-14, -8), offs + Vector2(14, 8), blade_col, 3.0)
			draw_line(offs + Vector2(-12, -7), offs + Vector2(12, 7), blade_inner, 1.0)
			# Zweite Klinge gekreuzt
			draw_line(offs + Vector2(-14, 8), offs + Vector2(14, -8), blade_col, 3.0)
			draw_line(offs + Vector2(-12, 7), offs + Vector2(12, -7), blade_inner, 1.0)
			# Funke in der Mitte (kleiner pulsierender Kreis)
			var pulse: float = 0.5 + 0.5 * sin(t_age * 18.0)
			draw_circle(offs, 3.0 + pulse * 1.2, Color(1.0, 0.9, 0.3, fade * 0.6))
	var flash   = _hit_flash > 0
	# -- South Park Stil --
	var skin      = Color(0.98, 0.82, 0.66) if not flash else Color(1, 1, 1)
	var tank      = Color(0.95, 0.92, 0.88)   # weisses Tank-Top
	var pants     = Color(0.22, 0.26, 0.40)    # dunkle Jeans
	var suspender = Color(0.32, 0.18, 0.06)    # dunkle Leder-Hosentraeger
	...
	# Tank-Top Torso (weiss)
	draw_rect(Rect2(-11, -8 + bob, 22, 22), tank)

	# Hosentraeger (ueber Tank-Top)
	...
	# Strohhut (breit, Suedstaaten-Stil)
...
func _on_kill_passive(_enemy) -> void:
	# Blade Momentum: Kill schleudert 2 extra Klingen in zufaellige Richtungen
	# Fuehlt sich an als wuerden die Klingen durch den Kill Energie aufnehmen
	for i in range(2):
		var angle = randf() * TAU
		spawn_projectile(Vector2(cos(angle), sin(angle)), get_total_damage() * 0.6, 500.0, 1)

func _use_ultimate() -> void:
	# Shred Solo: 8 String-Blades in alle Richtungen, jede bounct 2x von Screen-Edges
	var ult_damage: float = get_total_damage() * 3.5
	if has_upgrade("power_chord"):
		ult_damage *= 1.4
	for i in range(8):
		var angle: float = float(i) * TAU / 8.0 + _anim_time * 0.3
		var dir: Vector2 = Vector2(cos(angle), sin(angle))
		# spawn_projectile setzt selber bounce_count = extra_bounce, daher
		# nutzen wir _PROJ_SCENE direkt um die 2 Bounces explizit zu setzen
		var proj = _PROJ_SCENE.instantiate()
		proj.global_position = global_position
		proj.direction = dir
		proj.damage = ult_damage
		proj.speed = 650.0
		proj.pierce_count = self.pierce + 3
		proj.bounce_count = 2 + extra_bounce
		proj.shooter = self
		proj.proj_type = 3   # Riff Slicer Pick
